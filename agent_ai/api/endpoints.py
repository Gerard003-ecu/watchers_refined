...
def get_status():
    ...
    try:
        ...
        return jsonify({"status": "error", "mensaje": "Error interno al obtener estado"}), 500
...
def post_command():
    ...
    if not comando:
        ...
        return jsonify({"status": "error", "mensaje": "Falta el campo 'comando'"}), 400

    try:
        # Delegar el procesamiento al método de comandos estratégicos
        resultado = agent_ai_instance_app.actualizar_comando_estrategico(comando, valor)
        # Determinar código de estado basado en el resultado
        status_code = 200 if resultado.get("status") == "success" else 400
        return jsonify(resultado), status_code
    except Exception as e:
        logger.exception(f"Error al procesar comando estratégico: {comando}")
        return jsonify({"status": "error", "mensaje": f"Error interno al procesar comando '{comando}'"}), 500
...
def register_module():
    ...
    try:
        ...
        return jsonify({"status": "error", "mensaje": "Error interno al registrar módulo"}), 500
...
def health():
    ...
    try:
        ...
        is_loop_alive = (hasattr(agent_ai_instance_app, '_strategic_thread') and
                         agent_ai_instance_app._strategic_thread is not None and
                         agent_ai_instance_app._strategic_thread.is_alive())

        health_status = "success" if is_loop_alive else "error"
        message = "AgentAI operativo." if is_loop_alive else "Error: Bucle estratégico no está activo."
        ...
        return jsonify({"status": "error", "mensaje": "Error interno al verificar salud"}), 500
...
def control_input():
    ...
    try:
        ...
        agent_ai_instance_app.recibir_control_cogniboard(control_signal) # Pasar la señal
        return jsonify({"status": "success", "mensaje": "Señal de control recibida"}), 200
    except Exception as e:
        logger.exception("Error al procesar señal de control externa")
        return jsonify({"status": "error", "mensaje": "Error interno al procesar señal de control"}), 500
...
def config_input():
    ...
    if config_status is None:
        ...
        return jsonify({"status": "error", "mensaje": "Falta 'config_status'"}), 400

    try:
        logger.info(f"Recibido estado de configuración: {config_status}")
        agent_ai_instance_app.recibir_config_status(config_status) # Pasar el estado
        return jsonify({"status": "success", "mensaje": "Estado de configuración recibido"}), 200
    except Exception as e:
        logger.exception("Error al procesar estado de configuración externa")
        return jsonify({"status": "error", "mensaje": "Error interno al procesar estado de configuración"}), 500

# --- Punto de Entrada (si se ejecuta como script) ---
if __name__ == "__main__":
    port = int(os.environ.get('PORT', 9000)) # Usar PORT genérico si AGENT_AI_PORT no está
    ...
    app.run(host="0.0.0.0", port=port, debug=False, use_reloader=False) # Mantener para desarrollo
