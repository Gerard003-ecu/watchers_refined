...
# --- Configuración ---
HARMONY_CONTROLLER_URL = os.environ.get("HARMONY_CONTROLLER_URL", "http://harmony_controller:7000")
HARMONY_CONTROLLER_REGISTER_URL = os.environ.get("HARMONY_CONTROLLER_REGISTER_URL", f"{HARMONY_CONTROLLER_URL}/api/harmony/register_tool")
...
class AgentAI:
    def __init__(self):
        ...
        try:
            initial_vector_str = os.environ.get('AA_INITIAL_SETPOINT_VECTOR', '[1.0, 0.0]')
            parsed_vector = json.loads(initial_vector_str)
            if isinstance(parsed_vector, list) and all(isinstance(x, (int, float)) for x in parsed_vector):
            ...
            logger.error(f"AA_INITIAL_SETPOINT_VECTOR ('{initial_vector_str}') inválido ({e}), usando default [1.0, 0.0]")
        ...
        self.central_urls["harmony_controller"] = hc_url if hc_url else DEFAULT_HC_URL # Usar default si es None o ""
        ...
        logger.info(f"URLs Centrales configuradas: HC='{self.central_urls['harmony_controller']}', ECU='{self.central_urls['ecu']}', Malla='{self.central_urls['malla_watcher']}'")
        # Leer URL de registro manejando explícitamente None o ""
        hc_reg_url_env = os.environ.get(HARMONY_CONTROLLER_REGISTER_URL_ENV)
        self.hc_register_url = hc_reg_url_env if hc_reg_url_env else f"{self.central_urls['harmony_controller']}/api/harmony/register_tool"
        logger.info(f"URL de registro de Harmony Controller: '{self.hc_register_url}'")
        # --- FIN NUEVO ---
        self._strategic_thread = threading.Thread(target=self._strategic_loop, daemon=True, name="AgentAIStrategicLoop")
    ...
    def _strategic_loop(self):
        ...
        for attempt in range(max_retries_hc_wait):
            try:
                response = requests.get(f"{HARMONY_CONTROLLER_URL}/api/harmony/state", timeout=REQUESTS_TIMEOUT)
                if response.status_code == 200:
                    logger.info("Conexión establecida con harmony_controller")
                    hc_ready = True
                    break
            except Exception as e:
                logger.debug(f"Intento {attempt+1}/{max_retries_hc_wait}: harmony_controller no disponible ({str(e)})")
                time.sleep(retry_interval_hc_wait)
        if not hc_ready:
             logger.warning("No se pudo establecer conexión inicial con harmony_controller.")
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                with self.lock:
                    setpoint_changed = not np.allclose(self.target_setpoint_vector, new_setpoint_vector, rtol=1e-5, atol=1e-8)
                    if setpoint_changed:
                        self.target_setpoint_vector = new_setpoint_vector
                        logger.info(f"Nuevo setpoint estratégico determinado: {[f'{x:.3f}' for x in self.target_setpoint_vector]}")
        ...
        for attempt in range(MAX_RETRIES):
            ...
            try:
                ...
                if response_data.get("status") == "success" and "data" in response_data:
                    logger.debug(f"Estado válido recibido de Harmony: {response_data['data']}")
                    return response_data["data"] # <-- Correcto: dentro de if/try/for/def
                else:
                    logger.warning(f"Respuesta inválida desde Harmony: {response_data}")

            # --- Orden de Excepts Revisado (o simplificado a Exception) ---
            except Exception as e:
                 logger.exception(f"Error inesperado (intento {attempt+1}): {e}")
            ...
            if attempt < MAX_RETRIES - 1:
                ...
                logger.debug(f"Reintentando obtener estado de Harmony en {delay:.2f}s...")
                time.sleep(delay)
        # --- FIN BLOQUE INDENTADO ---

        # Si el bucle termina sin éxito
        logger.error(f"No se pudo obtener estado válido de Harmony después de {MAX_RETRIES} intentos.")
    ...
    def _determine_harmony_setpoint(self, measurement, cogniboard_signal, config_status, strategy, modules) -> List[float]:
        """
        ...
        with self.lock:
            ...
            current_target_norm = np.linalg.norm(current_target_vector) if current_target_vector else 0.0
        ...
        logger.debug(f"[SetpointLogic] Norma Actual: {measurement:.3f}, Norma Objetivo: {current_target_norm:.3f}, Error Global: {error_global:.3f}")
        ...
        for mod_info in modules.values():
            if mod_info.get("tipo") == "auxiliar" and mod_info.get("estado_salud") == "ok":
        ...
        logger.debug(f"[SetpointLogic] Aux Activos - Malla(P:{aux_stats['malla']['potenciador']}, R:{aux_stats['malla']['reductor']}), "
                     f"ECU(P:{aux_stats['ecu']['potenciador']}, R:{aux_stats['ecu']['reductor']})")
        ...
        if strategy == "estabilidad":
            if abs(error_global) < stability_threshold or abs(last_pid_output) > pid_effort_threshold:
                norm_vec = np.linalg.norm(new_target_vector)
                if norm_vec > 1e-6:
                    logger.info("[Estrategia Estabilidad] Reduciendo magnitud setpoint.")
            ...
            if abs(error_global) < stability_threshold and abs(last_pid_output) < pid_effort_threshold / 2:
                norm_vec = np.linalg.norm(new_target_vector)
                if norm_vec > 1e-6:
                    logger.info("[Estrategia Rendimiento] Aumentando magnitud setpoint.")
                    new_target_vector = adjust_vector(new_target_vector, 1.02)
                elif norm_vec < 1e-6:
                    logger.info("[Estrategia Rendimiento] Estableciendo setpoint mínimo.")
            ...
            if aux_stats["ecu"]["potenciador"] > aux_stats["ecu"]["reductor"]:
                logger.info("[Rendimiento] Más potenciadores en ECU, aumento extra.")
            ...
            total_reductores = aux_stats["malla"]["reductor"] + aux_stats["ecu"]["reductor"]
            if total_reductores > 0:
                logger.info("[Ahorro Energía] Reductores activos, reducción de setpoint.")
        ...
        if cogniboard_signal is not None:
            try:
                ...
                if signal_val > 0.8:
                    logger.info("[Cogniboard] Señal alta detectada, reduciendo magnitud final.")
                    norm = np.linalg.norm(new_target_vector)
                    if norm > 1e-6:
                        new_target_vector = adjust_vector(new_target_vector, 0.9)
            except (ValueError, TypeError):
                logger.warning(f"No se pudo convertir la señal de cogniboard a float: {cogniboard_signal}")

        if not isinstance(new_target_vector, list):
            logger.error(f"Error interno: _determine_harmony_setpoint no generó una lista: {type(new_target_vector)}")
    ...
    def _send_setpoint_to_harmony(self, setpoint_vector: List[float]):
        ...
        logger.debug(f"Intentando enviar setpoint a HC: {setpoint_vector} a {url}") # Log inicial
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                logger.info(f"Setpoint {setpoint_vector} enviado exitosamente a HC. Respuesta: {response.status_code}")
                return # Salir de la función si el envío es exitoso

            except Exception as e: # Captura simplificada de cualquier excepción
                logger.error(f"Error al enviar setpoint a HC ({url}) intento {attempt+1}/{MAX_RETRIES}: {type(e).__name__} - {e}")
                # Lógica de espera para reintento (DENTRO DEL BUCLE)
                if attempt < MAX_RETRIES - 1:
                    delay = BASE_RETRY_DELAY * (2 ** attempt)
                    logger.debug(f"Reintentando envío de setpoint a HC en {delay:.2f}s...")
                    time.sleep(delay) # Esperar antes del siguiente intento
                # else: # No es necesario un else aquí, el log de fallo final va después del bucle

        # Si el bucle termina sin un 'return' exitoso:
        logger.error(f"No se pudo enviar setpoint {setpoint_vector} a HC después de {MAX_RETRIES} intentos.")
    ...
    def registrar_modulo(self, modulo_info):
        ...
        naturaleza_auxiliar = modulo_info.get("naturaleza_auxiliar") # Puede ser None

        # --- Validación de Datos de Registro ---
        valido, mensaje = validate_module_registration(modulo_info)
        if not valido:
            logger.error(f"Registro fallido para '{nombre}' (datos inválidos): {mensaje} - Data: {modulo_info}")
        ...
        if req_path:
            if not os.path.exists(req_path):
                ...
                try:
                    deps_ok, deps_msg = check_missing_dependencies(req_path, GLOBAL_REQUIREMENTS_PATH)
                    if not deps_ok:
                        logger.error(f"Registro fallido para '{nombre}' (dependencias): {deps_msg}")
                ...
                logger.warning(f"No se encontró GLOBAL_REQUIREMENTS_PATH en {GLOBAL_REQUIREMENTS_PATH}, omitiendo chequeo de dependencias para '{nombre}'")
        ...
        with self.lock:
            if nombre in self.modules:
                logger.warning(f"Intento de registrar módulo existente: {nombre}")
            ...
            if aporta_a: log_details += f", Aporta a: {aporta_a}"
            if naturaleza_auxiliar: log_details += f", Naturaleza: {naturaleza_auxiliar}"
            logger.info(f"Módulo '{nombre}' ({log_details}) registrado. {deps_msg}. Pendiente de validación de salud.")

        # Iniciar validación de salud
        threading.Thread(target=self._validar_salud_modulo, args=(nombre,), daemon=True, name=f"HealthCheck-{nombre}").start()
    ...
    def _validar_salud_modulo(self, nombre):
        ...
        with self.lock:
            ...
            if not modulo:
                logger.error(f"No se encontró el módulo '{nombre}' para validar salud (ya eliminado?).")
                return
        ...
        if not modulo_url_salud:
            logger.error(f"No se encontró URL de salud para validar módulo '{nombre}'")
            ...
            for attempt in range(MAX_RETRIES):
                try:
                    logger.debug(f"Validando salud de '{nombre}' en {modulo_url_salud}... (intento {attempt+1}/{MAX_RETRIES})")
                    response = requests.get(modulo_url_salud, timeout=REQUESTS_TIMEOUT)

                    if response.status_code == 200:
                        estado_salud = "ok"
                        logger.info(f"Módulo '{nombre}' validado exitosamente (Salud OK).")
                        break # Salir del bucle FOR si OK
                    else:
                        estado_salud = f"error_{response.status_code}"
                        logger.warning(f"Validación de salud fallida para '{nombre}'. Status: {response.status_code}")

                # --- Bloque Except MUY Simplificado ---
                except Exception as e:
                    # Asignar un estado genérico y loguear la excepción completa
                    estado_salud = "error_inesperado"
                    logger.exception(f"Error inesperado al validar salud de '{nombre}': {e}")
                ...
                if attempt < MAX_RETRIES - 1:
                    delay = BASE_RETRY_DELAY * (2 ** attempt)
                    logger.debug(f"Reintentando validación de salud para '{nombre}' en {delay:.2f}s...")
                    time.sleep(delay)
                else:
                    logger.error(f"Validación de salud para '{nombre}' falló después de {MAX_RETRIES} intentos. Último estado: {estado_salud}")
        ...
        with self.lock:
            if nombre in self.modules:
                self.modules[nombre]["estado_salud"] = estado_salud
                 ...
                 logger.warning(f"Módulo '{nombre}' desapareció antes de actualizar estado de salud.")
                 return
        ...
        if (estado_salud == "ok" and
            ...
            if modulo_url_control:
                logger.info(f"Módulo auxiliar '{nombre}' saludable. Notificando a Harmony Controller...")
                ...
                logger.error(f"Módulo auxiliar '{nombre}' saludable pero no tiene URL de control definida. No se puede notificar a HC.")
        elif estado_salud == "ok" and modulo_tipo == "auxiliar":
             if not modulo_aporta_a:
                 logger.warning(f"Módulo auxiliar '{nombre}' saludable pero no declaró 'aporta_a'. No se notificará a HC.")
             if not modulo_naturaleza:
                 logger.warning(f"Módulo auxiliar '{nombre}' saludable pero no declaró 'naturaleza_auxiliar'. No se notificará a HC.")

    ### NUEVO: Función para notificar a HC sobre un nuevo tool auxiliar y naturaleza ###
    def _notify_harmony_controller_of_tool(self, nombre: str, url: str, aporta_a: str, naturaleza: str):
        # Usar la URL de registro almacenada
        register_url = self.hc_register_url
        payload = { ... }
        for attempt in range(MAX_RETRIES):
            try:
                logger.debug(f"Notificando a HC sobre '{nombre}' (Naturaleza: {naturaleza}) en {register_url} (intento {attempt+1}/{MAX_RETRIES})")
                response = requests.post(register_url, json=payload, timeout=REQUESTS_TIMEOUT)
                response.raise_for_status()
                logger.info(f"Notificación para '{nombre}' enviada exitosamente a HC. Respuesta: {response.status_code}")
                return # Éxito -> Salir de la función
                # --- Bloque Except MUY Simplificado ---
            except Exception as e:
               logger.exception(f"Error inesperado: {e}")         # --- Fin Bloque Except ---
                # --- Lógica de Espera/Reintento (DENTRO DEL BUCLE) ---
            if attempt < MAX_RETRIES - 1:
                delay = BASE_RETRY_DELAY * (2 ** attempt)
                logger.debug(f"Reintentando notificación a HC para '{nombre}' en {delay:.2f}s...")
                time.sleep(delay) # 
            else:
                logger.error(f"No se pudo notificar a HC sobre '{nombre}' después de {MAX_RETRIES} intentos.")
        ...
        with self.lock:
            if comando == "set_strategy":
                if isinstance(valor, str):
                    ...
                    return {"status": "success", "mensaje": f"Estrategia establecida a '{valor}'"}
                else:
                    return {"status": "error", "mensaje": "Valor para 'set_strategy' debe ser un string"}
            elif comando == "set_target_setpoint_vector":
                 try:
                     if valor is None:
                          return {"status": "error", "mensaje": "Falta 'valor' para 'set_target_setpoint_vector'"}
                     new_vector = [float(x) for x in valor]
                     self.target_setpoint_vector = new_vector
                     logger.info(f"Setpoint objetivo directo actualizado a: {new_vector}")
                     self._send_setpoint_to_harmony(self.target_setpoint_vector)
                     return {"status": "success", "mensaje": f"Setpoint objetivo establecido a {new_vector}"}
                 except (ValueError, TypeError):
                     return {"status": "error", "mensaje": "Valor para 'set_target_setpoint_vector' debe ser una lista de números"}
            else:
                logger.warning(f"Comando estratégico desconocido recibido: {comando}")
                return {"status": "error", "mensaje": f"Comando estratégico '{comando}' no reconocido"}
    ...
    def recibir_control_cogniboard(self, control_signal):
        ...
        logger.debug(f"Señal de control de Cogniboard actualizada: {control_signal}")
    ...
    def obtener_estado_completo(self):
        ...
        return {
            ...
            "registered_modules": modules_list # Incluye ahora 'naturaleza_auxiliar' si aplica
        }
