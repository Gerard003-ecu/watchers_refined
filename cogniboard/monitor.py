...
Este script actúa como el "cogniboard", la torre de control del ecosistema
watchers. Monitorea los endpoints de salud de los servicios, calcula una
métrica de entropía (definida como el número de servicios que no responden),
aplica un controlador PID para generar una señal de control y envía dicha
señal a agent_ai para que realice acciones correctivas.

Además, este script expone un endpoint de salud (/api/health) mediante
Flask para que podman pueda verificar el estado del servicio.

Variables de entorno:
    CONTROL_INTERVAL: Intervalo (segundos) entre ciclos (default: 30)
    PID_KP, PID_KI, PID_KD: Parámetros PID (default: 1.0, 0.1, 0.05)
    IDEAL_ENTROPY: Entropía deseada (idealmente 0, sistema sin fallos)
"""
...
def fetch_health(url):
    """
    Realiza una solicitud GET al endpoint de salud y retorna True si responde
    correctamente.
    """
    try:
...
def monitor_loop():
    ...
    Bucle principal de monitoreo:
      ...
      2. Usa un PID para calcular la señal de control basada en la diferencia
         entre la entropía medida y la deseada (IDEAL_ENTROPY).
...
if __name__ == "__main__":
    # Ejecutar el monitor en un hilo y lanzar el servidor Flask para el
    # healthcheck.
    import threading
