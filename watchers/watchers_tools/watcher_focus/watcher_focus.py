    ...
    for attempt in range(MAX_REGISTRATION_RETRIES):
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"Registro de '{module_name}' recibido con status "
...
def set_focus_control():
    ...
    try:
        ...
        return jsonify(
            {"status": "error", "message": "control_signal debe ser un número"}
        ), 400
    except Exception:
        logger.exception("Error al aplicar señal de control a WatcherFocus")
        return jsonify(
            {"status": "error", "message": "Error interno al aplicar control"}
        ), 500
...
if __name__ == "__main__":
    ...
    NATURALEZA = "reductor"  # ¿Reduce la oscilación? ¿O la modula?
