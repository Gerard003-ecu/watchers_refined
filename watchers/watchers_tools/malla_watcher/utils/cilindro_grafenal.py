...
class HexCylindricalMesh:
    ...
    def __init__(self,
        ...
        if hex_width_circumferential < EPSILON:
            raise ValueError(
                ...
                "resultando en un ancho circunferencial nulo."
            )
        ...
        if not math.isclose(
            self.actual_circumference_covered_by_q_segments, self.circumference, rel_tol=0.15
    ...
    def _initialize_mesh(self):
        ...
        Las coordenadas (q,r) son axiales. 'q' se enrolla para la circunferencia,
        'r' contribuye a la altura Z.
        """
        logger.info(
        ...
        # Altura de una fila de hexágonos (distancia vertical entre centros de hexágonos
        # en la misma columna q, r vs r+1)
        ...
        # La altura de una "doble fila" (para cubrir un ciclo completo de patrón vertical)
        # es sqrt(3)*hex_size.
        ...
        if self.height_segments > 0:
            # self.total_height_approx = self.height_segments * (math.sqrt(3.0) / 2.0 * self.hex_size) # Subestima
        ...
        while queue and current_bfs_iteration < max_bfs_iterations:
            ...
            # x_flat, y_flat = axial_to_cartesian_flat(q_for_x_flat, r, self.hex_size) # No usada directamente
            ...
            cyl_radius_calc, cyl_theta_calc, cyl_z_calc = cartesian_flat_to_cylindrical(
            ...
            if is_within_strict_z:
                if axial_key not in self.cells:
                    ...
                    if cells_added_count % 100 == 0:
                        logger.debug(
                            f"--> Celda añadida #{cells_added_count}: {axial_key} (z={cyl_z_calc:.2f})"
                        )
            ...
            for nq, nr in theoretical_neighbors:
                ...
                if neighbor_key not in processed_coords:
                    # Verificar si el vecino potencial está dentro de un rango q razonable para evitar
                    # exploración infinita si la circunferencia no cierra bien y no hay periodicidad
                    # en q en la generación.
                    # El límite de q_exploration_limit es para el q original, no el envuelto.
                    q_exploration_limit_factor = 2  # Permitir explorar hasta 2x la circunferencia en q
                    if abs(nq) > self.circumference_segments_actual * q_exploration_limit_factor:
                        # logger.debug(f"Saltando vecino ({nq},{nr}) por exceder límite q.")
                        continue
    ...
    def verify_connectivity(self, expected_min_neighbors_internal: int = 6) -> Dict[int, int]:
        ...
        logger.info(
            f"Verificando conectividad de {len(self.cells)} celdas (periodic_z={self.periodic_z})..."
        )

        for _cell_key, cell in self.cells.items():  # cell_key no se usa
            ...
            # expected_neighbors_for_this_cell = expected_min_neighbors_internal # No se usa con este nombre
            if is_on_z_border:
        ...
        logger.info(
            ...
            f"Distribución (NumVecinos: NumCeldas): {result_dict}"
        )
        if self.cells:  # Evitar división por cero si no hay celdas
            percentage_low_connectivity = cells_with_few_neighbors * 100 / len(self.cells)
            logger.info(
        ...
        if max_neighbors_found > 6 and self.cells:
            logger.error(
                "¡ERROR CRÍTICO DE CONECTIVIDAD! Se encontraron celdas con más de 6 vecinos."
            )
    ...
    def get_neighbor_cells(self, q_center: int, r_center: int) -> List[Cell]:
        ...
        # if not center_cell:
        #     logger.warning(
        #         f"get_neighbor_cells: Celda central ({q_center},{r_center}) no existe."
        #     )
        #     return []
        ...
        for nq_orig, nr_orig in theoretical_neighbor_coords:
            ...
            if self.periodic_z and self.total_height_approx > EPSILON:
                ...
                x_flat_theoretical_neighbor, y_flat_theoretical_neighbor = axial_to_cartesian_flat(
                    nq_orig, nr_orig, self.hex_size
                )
                _, _, z_theoretical_neighbor = cartesian_flat_to_cylindrical(
                    x_flat_theoretical_neighbor, y_flat_theoretical_neighbor, self.radius
                )
    ...
    def _find_closest_z_neighbor(self, target_q_original: int, target_z_theoretical: float) -> Optional[Cell]:
        ...
        "versión envuelta en Z" de un vecino teórico
        (target_q_original, target_r_donde_sea_que_este_target_z).
        ...
        # logger.debug(
        #   f"    Buscando Z-vecino para q_orig={target_q_original} "
        #   f"(envuelto={target_q_wrapped_for_comparison}), z_teor={target_z_theoretical:.2f}"
        # )

        if self.max_z - self.min_z <= EPSILON:  # Altura real de la malla es cero o despreciable
            # logger.debug(
            #   "    Altura real de la malla es cero, no se pueden encontrar vecinos envueltos en Z."
            # )
            return None
        ...
        for cell_candidate in self.cells.values():
            ...
            if effective_dz < min_dz_abs_effective and effective_dz < z_match_tolerance:
                ...
                # logger.debug(
                #   f"      -> Nuevo mejor match Z-vecino: ({best_match.q_axial},{best_match.r_axial}) "
                #   f"z={best_match.z:.2f} con dz_eff={min_dz_abs_effective:.4f}"
                # )

        # if best_match:
        #     logger.debug(
        #        f"    Encontrado Z-vecino: ({best_match.q_axial},{best_match.r_axial}) para "
        #        f"q_orig={target_q_original}, z_teor={target_z_theoretical:.2f}"
        #     )
        # else:
        #     logger.debug(
        #        f"    No se encontró Z-vecino para q_orig={target_q_original}, "
        #        f"z_teor={target_z_theoretical:.2f} (min_dz_eff={min_dz_abs_effective:.2f} "
        #        f"> tol={z_match_tolerance:.2f})"
        #     )
