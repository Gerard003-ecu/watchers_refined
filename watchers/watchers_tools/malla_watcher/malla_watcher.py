...
def apply_external_field_to_mesh(mesh_instance: HexCylindricalMesh, field_vector_map: List[List[List[float]]]):
    ...
    logger.debug(
        f"Aplicando campo vectorial externo (shape={field_vector_np.shape}) "
        "a la malla cilíndrica usando interpolación bilineal..."
    )
...
def update_aggregate_state():
    ...
    logger.debug(
        f"Estado agregado actualizado: AvgAmp={avg_amp:.3f}, MaxAmp={max_amp:.3f}, "
        f"AvgVel={avg_vel:.3f}, MaxVel={max_vel:.3f}, AvgKE={avg_ke:.3f}, "
    ...
    if data.get("type") == "pulse" and "coords" in data and "magnitude" in data:
        ...
        if q_coord is not None and r_coord is not None:
            ...
            if cell:
                ...
                return jsonify({"status": "warning", "message": f"Celda ({q_coord},{r_coord}) no encontrada."}), 404  # Not Found
        else:
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Configuración Malla REAL: R={MESH_RADIUS_REAL}, HSeg={MESH_HEIGHT_SEGMENTS_REAL}, "
        f"CSeg={MESH_CIRCUMFERENCE_SEGMENTS_REAL}, Hex={MESH_HEX_SIZE_REAL}, "
        f"PerZ={MESH_PERIODIC_Z_REAL}"
    )
    logger.info(
    ...
    logger.info(
        f"Configuración Control REAL: BaseC={BASE_COUPLING_T_REAL}, BaseD={BASE_DAMPING_E_REAL}, "
        f"GainC={K_GAIN_COUPLING_REAL}, GainD={K_GAIN_DAMPING_REAL}"
    )
    logger.info(
