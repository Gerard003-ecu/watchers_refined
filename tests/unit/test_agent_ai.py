...
# Importar la clase y constantes/funciones necesarias desde la ubicación correcta
# Asumiendo que la estructura es mi-proyecto/agent_ai/agent_ai.py
from agent_ai.agent_ai import (
    AgentAI,
    # Constantes de URL existentes y nuevas
...
class TestAgentAIStrategicLogic(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if hasattr(self.agent, '_strategic_thread') and self.agent._strategic_thread.is_alive():
    ...
    def test_initialization(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        ...
        try:
            initial_vector = json.loads(os.environ.get('AA_INITIAL_SETPOINT_VECTOR', '[1.0, 0.0]'))
            if not (isinstance(initial_vector, list) and all(isinstance(x, (int, float)) for x in initial_vector)):
        ...
        self.assertEqual(agent.current_strategy, os.environ.get('AA_INITIAL_STRATEGY', 'default'))
    ...
    def test_init_central_urls_defaults(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        """Verifica que __init__ usa URLs default si ENV no están definidas."""
        # Asegurar un entorno limpio para este test, sin las variables relevantes
        env_vars_to_clear = [
            HARMONY_CONTROLLER_URL_ENV,
            HARMONY_CONTROLLER_REGISTER_URL_ENV,
            AGENT_AI_ECU_URL_ENV,
        ...
        with mock.patch.dict(os.environ, {k: "" for k in env_vars_to_clear}, clear=True):
            # Crear instancia DENTRO del contexto del patch
            agent_test = AgentAI()

            # Verificar que las URLs almacenadas son las defaults
            self.assertEqual(agent_test.central_urls.get("harmony_controller"), DEFAULT_HC_URL)
            self.assertEqual(agent_test.central_urls.get("ecu"), DEFAULT_ECU_URL)
            self.assertEqual(agent_test.central_urls.get("malla_watcher"), DEFAULT_MALLA_URL)
            # Verificar que la URL de registro se construye con el default de HC
            self.assertEqual(agent_test.hc_register_url, f"{DEFAULT_HC_URL}/api/harmony/register_tool")

    def test_init_central_urls_from_env(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        ...
        with mock.patch.dict(os.environ, simulated_env, clear=True):
            ...
            self.assertEqual(agent_test.central_urls.get("harmony_controller"), test_hc_url)
            self.assertEqual(agent_test.central_urls.get("ecu"), test_ecu_url)
            self.assertEqual(agent_test.central_urls.get("malla_watcher"), test_malla_url)
    ...
    def test_get_harmony_state_success(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        """Prueba obtener estado de Harmony con éxito."""
        mock_response = mock.MagicMock()
        mock_response.status_code = 200
        mock_harmony_data = {"last_measurement": 0.5, "last_ecu_state": [[0.4], [0.3]]} # Ejemplo con lista de listas
        mock_response.json.return_value = {"status": "success", "data": mock_harmony_data}
        mock_requests.get.return_value = mock_response
        mock_requests.get.side_effect = None # Limpiar side effects previos
        state = self.agent._get_harmony_state()
        expected_url = f"{self.agent.central_urls.get('harmony_controller', DEFAULT_HC_URL)}/api/harmony/state"
    ...
    def test_get_harmony_state_network_error(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
    ...
    def test_get_harmony_state_bad_response(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
    ...
    def test_send_setpoint_to_harmony_success(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        ...
        expected_url = f"{self.agent.central_urls.get('harmony_controller', DEFAULT_HC_URL)}/api/harmony/setpoint"
    ...
    def test_send_setpoint_to_harmony_error(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
    ...
    def test_determine_harmony_setpoint_simple(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        ...
        np.testing.assert_allclose(new_sp, current_target, err_msg="Default: El setpoint no debe cambiar.")
        ...
        np.testing.assert_allclose(new_sp, expected_target, err_msg="Cogniboard: El setpoint no se redujo correctamente.")

        # Caso cogniboard señal inválida: sin cambios
        current_target = [1.0, 0.0]
        self.agent.target_setpoint_vector = list(current_target)
        self.agent.external_inputs["cogniboard_signal"] = "invalid"
        new_sp = self.agent._determine_harmony_setpoint(0.5, "invalid", None, "default", {})
        np.testing.assert_allclose(new_sp, current_target, err_msg="Cogniboard inválido: El setpoint no debe cambiar.")

        # Estrategia: estabilidad, reducción por error bajo o esfuerzo alto
        self.agent.target_setpoint_vector = [2.0, 0.0]
        modules = {}
        # Simular error bajo
        new_sp = self.agent._determine_harmony_setpoint(1.99, None, None, "estabilidad", modules)
        self.assertTrue(np.linalg.norm(new_sp) < np.linalg.norm([2.0, 0.0]), "Estabilidad: Debe reducir magnitud.")

        # Estrategia: estabilidad, reducción extra por más reductores que potenciadores
        modules = {
            "aux1": {"tipo": "auxiliar", "estado_salud": "ok", "aporta_a": "malla_watcher", "naturaleza_auxiliar": "reductor"},
            "aux2": {"tipo": "auxiliar", "estado_salud": "ok", "aporta_a": "malla_watcher", "naturaleza_auxiliar": "reductor"},
            "aux3": {"tipo": "auxiliar", "estado_salud": "ok", "aporta_a": "malla_watcher", "naturaleza_auxiliar": "potenciador"},
        }
        self.agent.target_setpoint_vector = [2.0, 0.0]
        new_sp = self.agent._determine_harmony_setpoint(1.99, None, None, "estabilidad", modules)
        # Debe reducir más que solo por el primer ajuste
        self.assertTrue(np.linalg.norm(new_sp) < 2.0 * 0.98, "Estabilidad: Reducción extra por reductores.")

        # Estrategia: rendimiento, aumento por error bajo y esfuerzo bajo
        self.agent.target_setpoint_vector = [2.0, 0.0]
        self.agent.harmony_state['last_pid_output'] = 0.1
        new_sp = self.agent._determine_harmony_setpoint(1.99, None, None, "rendimiento", {})
        self.assertTrue(np.linalg.norm(new_sp) > 2.0, "Rendimiento: Debe aumentar magnitud.")

        # Estrategia: rendimiento, aumento extra por más potenciadores en ECU
        modules = {
            "aux1": {"tipo": "auxiliar", "estado_salud": "ok", "aporta_a": "matriz_ecu", "naturaleza_auxiliar": "potenciador"},
            "aux2": {"tipo": "auxiliar", "estado_salud": "ok", "aporta_a": "matriz_ecu", "naturaleza_auxiliar": "potenciador"},
            "aux3": {"tipo": "auxiliar", "estado_salud": "ok", "aporta_a": "matriz_ecu", "naturaleza_auxiliar": "reductor"},
        }
        self.agent.target_setpoint_vector = [2.0, 0.0]
        self.agent.harmony_state['last_pid_output'] = 0.1
        new_sp = self.agent._determine_harmony_setpoint(1.99, None, None, "rendimiento", modules)
        # Debe aumentar más que solo por el primer ajuste
        self.assertTrue(np.linalg.norm(new_sp) > 2.0 * 1.02, "Rendimiento: Aumento extra por potenciadores ECU.")

        # Estrategia: ahorro_energia, reducción si hay reductores activos
        modules = {
            "aux1": {"tipo": "auxiliar", "estado_salud": "ok", "aporta_a": "malla_watcher", "naturaleza_auxiliar": "reductor"},
            "aux2": {"tipo": "auxiliar", "estado_salud": "ok", "aporta_a": "matriz_ecu", "naturaleza_auxiliar": "reductor"},
        }
        self.agent.target_setpoint_vector = [2.0, 0.0]
        new_sp = self.agent._determine_harmony_setpoint(1.99, None, None, "ahorro_energia", modules)
        self.assertTrue(np.linalg.norm(new_sp) < 2.0, "Ahorro energía: Debe reducir magnitud si hay reductores.")

        # --- NUEVOS TESTS DETALLADOS --- #
    def test_determine_estrategia_default_sin_cambio_base(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        ...
        self.agent.harmony_state = {'setpoint_value': np.linalg.norm(initial_vector), 'last_pid_output': 0.1}
        modules = {} # Sin auxiliares activos
        measurement = np.linalg.norm(initial_vector) # Simular medición igual al setpoint

        new_sp = self.agent._determine_harmony_setpoint(measurement, None, None, "default", modules)

        np.testing.assert_allclose(new_sp, initial_vector, err_msg="Default: Setpoint no debería cambiar.")

    def test_determine_estrategia_estabilidad_reduce_por_error_bajo(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        ...
        self.agent.target_setpoint_vector = list(initial_vector)
        self.agent.harmony_state = {'setpoint_value': initial_norm, 'last_pid_output': 0.1} # PID bajo
        modules = {}
        measurement = initial_norm * 0.95 # Error bajo (measurement cercano a setpoint)

        new_sp = self.agent._determine_harmony_setpoint(measurement, None, None, "estabilidad", modules)
        final_norm = np.linalg.norm(new_sp)

        self.assertTrue(final_norm < initial_norm, "Estabilidad/Error Bajo: Norma debería reducirse.")
        # Verificar que la reducción es la esperada (0.98)
        np.testing.assert_allclose(final_norm, initial_norm * 0.98, rtol=1e-6)
    
    def test_determine_estrategia_estabilidad_reduce_por_pid_alto(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        """Verifica que 'estabilidad' reduce magnitud si el esfuerzo PID es alto."""
        initial_vector = [2.0, 0.0]
        initial_norm = np.linalg.norm(initial_vector)
        self.agent.target_setpoint_vector = list(initial_vector)
        self.agent.harmony_state = {'setpoint_value': initial_norm, 'last_pid_output': 0.8} # PID alto
        modules = {}
        measurement = initial_norm * 0.5 # Error grande (no debería importar para esta condición)

        new_sp = self.agent._determine_harmony_setpoint(measurement, None, None, "estabilidad", modules)
        final_norm = np.linalg.norm(new_sp)

        self.assertTrue(final_norm < initial_norm, "Estabilidad/PID Alto: Norma debería reducirse.")
        np.testing.assert_allclose(final_norm, initial_norm * 0.98, rtol=1e-6)
    
    def test_determine_estrategia_estabilidad_reduce_extra_por_reductores(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        ...
        self.agent.harmony_state = {'setpoint_value': initial_norm, 'last_pid_output': 0.1} # PID bajo
        modules = {
            "r1": {"tipo": "auxiliar", "estado_salud": "ok", "aporta_a": "malla_watcher", "naturaleza_auxiliar": "reductor"},
            "r2": {"tipo": "auxiliar", "estado_salud": "ok", "aporta_a": "malla_watcher", "naturaleza_auxiliar": "reductor"},
            "p1": {"tipo": "auxiliar", "estado_salud": "ok", "aporta_a": "malla_watcher", "naturaleza_auxiliar": "potenciador"},
        }
        ...
        self.assertTrue(final_norm < initial_norm * 0.98, "Estabilidad/Reductores: Reducción extra esperada.")
        np.testing.assert_allclose(final_norm, expected_norm, rtol=1e-6)

    def test_determine_estrategia_rendimiento_aumenta(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        ...
        new_sp = self.agent._determine_harmony_setpoint(measurement, None, None, "rendimiento", modules)
        final_norm = np.linalg.norm(new_sp)

        self.assertTrue(final_norm > initial_norm, "Rendimiento/Estable: Norma debería aumentar.")
        np.testing.assert_allclose(final_norm, initial_norm * 1.02, rtol=1e-6)

    def test_determine_estrategia_rendimiento_aumenta_extra_por_potenciadores(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        ...
        self.agent.harmony_state = {'setpoint_value': initial_norm, 'last_pid_output': 0.1} # PID bajo
        modules = {
            "p1": {"tipo": "auxiliar", "estado_salud": "ok", "aporta_a": "matriz_ecu", "naturaleza_auxiliar": "potenciador"},
            "p2": {"tipo": "auxiliar", "estado_salud": "ok", "aporta_a": "matriz_ecu", "naturaleza_auxiliar": "potenciador"},
            "r1": {"tipo": "auxiliar", "estado_salud": "ok", "aporta_a": "matriz_ecu", "naturaleza_auxiliar": "reductor"},
        }
        measurement = initial_norm * 0.99 # Error bajo
        ...
        self.assertTrue(final_norm > initial_norm * 1.02, "Rendimiento/Potenciadores: Aumento extra esperado.")
        np.testing.assert_allclose(final_norm, expected_norm, rtol=1e-6)

    def test_determine_estrategia_rendimiento_establece_minimo_si_cero(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
    ...
    def test_determine_estrategia_ahorro_reduce_con_reductores(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        """Verifica que 'ahorro_energia' reduce magnitud si hay reductores."""
        initial_vector = [2.0, 0.0]
        initial_norm = np.linalg.norm(initial_vector)
        self.agent.target_setpoint_vector = list(initial_vector)
        self.agent.harmony_state = {'setpoint_value': initial_norm, 'last_pid_output': 0.1}
        modules = {
            "r1": {"tipo": "auxiliar", "estado_salud": "ok", "aporta_a": "malla_watcher", "naturaleza_auxiliar": "reductor"}
        }
        measurement = initial_norm # Sin error

        new_sp = self.agent._determine_harmony_setpoint(measurement, None, None, "ahorro_energia", modules)
        final_norm = np.linalg.norm(new_sp)

        self.assertTrue(final_norm < initial_norm, "Ahorro/Reductores: Norma debería reducirse.")
        np.testing.assert_allclose(final_norm, initial_norm * 0.95, rtol=1e-6)

    def test_determine_estrategia_ahorro_sin_cambio_sin_reductores(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        ...
        np.testing.assert_allclose(new_sp, initial_vector, err_msg="Ahorro/Sin Reductores: Setpoint no debería cambiar.")

    def test_determine_cogniboard_alto_reduce(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        ...
        new_sp = self.agent._determine_harmony_setpoint(measurement, cogniboard_signal, None, "default", modules)
        final_norm = np.linalg.norm(new_sp)

        self.assertTrue(final_norm < initial_norm, "Cogniboard Alto: Norma debería reducirse.")
    ...
    def test_registrar_modulo_auxiliar_success(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        ...
        mock_check_deps.assert_called_once_with("dummy_req.txt", GLOBAL_REQUIREMENTS_PATH)

    def test_registrar_modulo_central_success(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
    ...
    def test_registrar_modulo_invalid_data(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        """Prueba registro con datos faltantes (validator se encarga)."""
        # Asumiendo que validator.validate_module_registration verifica campos requeridos
        module_data = {"nombre": "TestReg"} # Falta url, tipo, etc.
        # Mockear el validador para simular fallo
        with mock.patch('agent_ai.agent_ai.validate_module_registration', return_value=(False, "Faltan campos requeridos: url, tipo")):
            result = self.agent.registrar_modulo(module_data)
        self.assertEqual(result["status"], "error")
        self.assertIn("Faltan campos requeridos", result["mensaje"])
        self.assertNotIn("TestReg", self.agent.modules)

    def test_registrar_modulo_dep_fail(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        """Prueba registro con fallo de dependencias."""
        mock_check_deps.return_value = (False, "Falta 'superlib'")
        mock_os_exists.return_value = True
        module_data = {
            "nombre": "TestDepFail", "url": "http://testdep/health", "tipo": "auxiliar",
    ...
    def test_validar_salud_modulo_ok_auxiliar_sin_naturaleza(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        ...
        self.agent.modules[module_name] = {
            "nombre": module_name, "url": module_url, "url_salud": module_url,
            "tipo": "auxiliar", "aporta_a": "malla_watcher", # Falta naturaleza
    ...
    def test_validar_salud_modulo_ok_central(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
    ...
    def test_validar_salud_modulo_fail_y_no_notifica(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        """Prueba validación fallida (no debe notificar)."""
        module_name = "FailAux"
        module_url = "http://failaux/api"
        self.agent.modules[module_name] = {
            "nombre": module_name, "url": module_url, "url_salud": module_url,
            "tipo": "auxiliar", "aporta_a": "matriz_ecu", "naturaleza_auxiliar": "potenciador",
            "estado_salud": "pendiente"
        }
        ...
        self.assertEqual(self.agent.modules[module_name]["estado_salud"], "error_inesperado")
        # Verificar que reintentó
        self.assertEqual(mock_requests.get.call_count, MAX_RETRIES)
        # Verificar que NO notificó
        mock_requests.post.assert_not_called()

    def test_notify_hc_retry_and_fail(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        ...
        self.agent._notify_harmony_controller_of_tool(
            nombre="NotifyFail", url="http://someurl", aporta_a="malla", naturaleza="modulador"
        )
    ...
    def test_actualizar_comando_estrategico(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        """Prueba comandos estratégicos."""
        # Set strategy
        result = self.agent.actualizar_comando_estrategico("set_strategy", "performance")
        self.assertEqual(result["status"], "success")
        ...
        new_vec = [2.0, 1.0]
        # -------------------------------------------------
        result = self.agent.actualizar_comando_estrategico("set_target_setpoint_vector", new_vec) # Ahora new_vec existe
        self.assertEqual(result["status"], "success")
    ...
    def test_recibir_inputs_externos(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        ...
        self.assertEqual(self.agent.external_inputs["config_status"], config_data)

    ### MODIFICADO: test_obtener_estado_completo incluye naturaleza ###
    def test_obtener_estado_completo(self, mock_thread, mock_os_exists, mock_check_deps, mock_requests):
        """Prueba obtener el estado completo con detalles de módulo."""
        self.agent.modules["TestModAux"] = {
            "nombre": "TestModAux", "estado_salud": "ok", "tipo": "auxiliar",
            "aporta_a": "malla_watcher", "naturaleza_auxiliar": "modulador"
        }
        self.agent.modules["TestModCentral"] = {
            "nombre": "TestModCentral", "estado_salud": "error_timeout", "tipo": "integrador"
        }
        self.agent.harmony_state = {"last_measurement": 0.9, "last_ecu_state": [[0.8], [0.1]]}
        ...
        self.assertEqual(estado["harmony_controller_last_state"]["last_measurement"], 0.9)
        self.assertEqual(len(estado["registered_modules"]), 2)

        # Verificar detalles del módulo auxiliar
        mod_aux = next(m for m in estado["registered_modules"] if m["nombre"] == "TestModAux")
        ...
        mod_central = next(m for m in estado["registered_modules"] if m["nombre"] == "TestModCentral")
