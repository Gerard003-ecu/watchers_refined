...
class PistonConfig:
    ...
    def __init__(self):
        ...
        self.simulation_interval = float(
            os.environ.get("ATOMIC_PISTON_SIM_INTERVAL", 0.02)
        )  # 50Hz
        self.agent_ai_register_url = os.environ.get(
            "AGENT_AI_REGISTER_URL", "http://agent_ai:9000/api/register"
        )
        self.requests_timeout = float(
            os.environ.get("ATOMIC_PISTON_REQUESTS_TIMEOUT", 3.0)
        )
        self.max_registration_retries = int(
            os.environ.get("MAX_REGISTRATION_RETRIES", 5)
        )
