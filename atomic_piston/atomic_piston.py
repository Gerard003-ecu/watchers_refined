...
class AtomicPiston:
    ...
    def __init__(self,
        ...
        self.capacitor_discharge_threshold = -capacity * 0.9  # Umbral de descarga para modo capacitor
        self.battery_is_discharging = False                   # Estado de descarga para modo batería
        self.battery_discharge_rate = capacity * 0.05         # Tasa de descarga en modo batería
        self.hysteresis_factor = 0.1                          # Factor de histéresis para evitar ciclado rápido
        ...
        logger.info(f"AtomicPiston inicializado en modo {mode.value} con transductor {transducer_type.value}")
    ...
    def update_state(self, dt: float):
        ...
        # Guardar posición actual para cálculo de velocidad
        ...
        new_position = 2 * self.position - self.previous_position + self.acceleration * (dt ** 2)
    ...
    def set_compression_direction(self, direction: int):
        ...
        if direction not in (-1, 1):
            logger.warning(f"Dirección de compresión inválida: {direction}. Usando -1 (compresión).")
    ...
    def trigger_discharge(self, discharge_on: bool):
        ...
        if self.mode == PistonMode.BATTERY:
            ...
            logger.warning(f"trigger_discharge llamado en modo {self.mode.value}. Solo tiene efecto en modo BATTERY.")
