...
- Recibe notificaciones de registro de AgentAI para tools auxiliares gestionados (incluyendo naturaleza). ### MODIFICADO ###
- Recibe un setpoint (objetivo de armonía) de AgentAI.
- Calcula señales de control (aún no diferenciadas semánticamente) para cada watcher_tool gestionado. ### MODIFICADO ###
...
#     logger.info(f"Watchers Tools iniciales (config): {list(MANAGED_TOOLS_INIT.keys())}")
# except json.JSONDecodeError:
#     logger.warning(f"No se proporcionó WATCHERS_TOOLS_CONFIG o es inválido. HC esperará registros de AgentAI.")
#     MANAGED_TOOLS_INIT = {}
...
SETPOINT_VECTOR_JSON = os.environ.get('HC_SETPOINT_VECTOR', '[1.0, 0.0]') # Ajustado default
try:
    ...
    logger.info(f"Setpoint inicial (norma): {setpoint_init:.3f} (derivado de {setpoint_vector_init.tolist()})")
except (json.JSONDecodeError, ValueError):
    logger.error(f"Error al procesar HC_SETPOINT_VECTOR: {SETPOINT_VECTOR_JSON}. Usando setpoint=1.0")
    setpoint_vector_init = np.array([1.0, 0.0]) # Default consistente con AgentAI
    setpoint_init = 1.0
...
class HarmonyControllerState:
    def __init__(self, kp=KP_INIT, ki=KI_INIT, kd=KD_INIT, initial_setpoint=setpoint_init, initial_setpoint_vector=setpoint_vector_init):
        self.pid_controller = BosonPhase(kp, ki, kd, setpoint=initial_setpoint)
        self.current_setpoint = initial_setpoint
        self.setpoint_vector = initial_setpoint_vector.tolist() if isinstance(initial_setpoint_vector, np.ndarray) else initial_setpoint_vector
        self.last_ecu_state: List[List[float]] = []
        ### MODIFICADO: Estructura para almacenar detalles, incluyendo naturaleza ###
        # Clave: nombre_tool, Valor: Dict{'url': str, 'aporta_a': str, 'naturaleza': str, 'last_state': Any, 'last_control': float}
    ...
    def update_setpoint(self, new_setpoint_value: float, new_setpoint_vector: Optional[List[float]] = None):
        with self.lock:
            ...
            logger.info(f"Setpoint actualizado a: {self.current_setpoint:.3f}" + (f" desde vector {new_setpoint_vector}" if new_setpoint_vector else ""))


    ### MODIFICADO: register_managed_tool ahora acepta y almacena 'naturaleza' ###
    def register_managed_tool(self, nombre: str, url: str, aporta_a: str, naturaleza: str): ### MODIFICADO ###
        with self.lock:
            if nombre not in self.managed_tools_details:
                self.managed_tools_details[nombre] = {}
                logger.info(f"Registrando nuevo tool gestionado: '{nombre}' (URL: {url}, Aporta a: {aporta_a}, Naturaleza: {naturaleza})") ### MODIFICADO ###
            else:
                 logger.info(f"Actualizando información del tool gestionado: '{nombre}'")
            self.managed_tools_details[nombre]['url'] = url
            self.managed_tools_details[nombre]['aporta_a'] = aporta_a
            self.managed_tools_details[nombre]['naturaleza'] = naturaleza ### NUEVO ###
    ...
    def unregister_managed_tool(self, nombre: str):
         with self.lock:
             if nombre in self.managed_tools_details:
                 ...
                 logger.warning(f"Intento de eliminar tool no gestionado: '{nombre}'")
    ...
    def get_state_snapshot(self) -> Dict[str, Any]:
        with self.lock:
            ...
            return {
                ...
                "pid_gains": {"Kp": self.pid_controller.Kp, "Ki": self.pid_controller.Ki, "Kd": self.pid_controller.Kd}
            }
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if data.get("status") == "success":
                ...
                if isinstance(state_list, list) and all(isinstance(row, list) for row in state_list):
                    logger.debug(f"Estado ECU recibido: {len(state_list)}x{len(state_list[0]) if state_list else 0} puntos")
                    return state_list
                else:
                    logger.error(f"Clave 'estado_campo_unificado' encontrada pero no es lista de listas: {type(state_list)}")
            else:
                logger.warning(f"Respuesta de ECU no exitosa: {data.get('message', 'Formato desconocido')}")
        except BaseException as e:
            logger.error(f"Error al obtener/procesar estado de ECU ({url}) intento {attempt+1}: {type(e).__name__} - {e}")
...
def get_tool_state(tool_name: str, base_url: str) -> Dict[str, Any]:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            response.raise_for_status()
            data = response.json()
            logger.debug(f"Estado recibido de {tool_name}: {data.get('state', data)}")
            return data.get("state", {"status": "success", "raw_data": data})
        except Exception as e:
            logger.warning(f"Error al obtener estado de {tool_name} ({state_url}) intento {attempt+1}: {type(e).__name__} - {e}")
            if attempt < MAX_RETRIES - 1:
                delay = BASE_RETRY_DELAY * (2 ** attempt)
                time.sleep(delay)
    return {"status": "error", "message": f"No se pudo obtener estado después de {MAX_RETRIES} intentos"}
...
def send_tool_control(tool_name: str, base_url: str, control_signal: float):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            response = requests.post(control_url, json=payload, timeout=REQUESTS_TIMEOUT)
            response.raise_for_status()
            logger.info(f"Señal de control {control_signal:.3f} enviada a {tool_name}. Respuesta: {response.status_code}")
            return True
        except Exception as e:
            logger.warning(f"Error al enviar control a {tool_name} ({control_url}) intento {attempt+1}: {type(e).__name__} - {e}")
            if attempt < MAX_RETRIES - 1:
                delay = BASE_RETRY_DELAY * (2 ** attempt)
                time.sleep(delay)
    logger.error(f"No se pudo enviar señal de control a {tool_name} después de {MAX_RETRIES} intentos.")
    return False
...
def harmony_control_loop():
    ...
    while True:
        ...
        with controller_state.lock:
            ...
            managed_tools_loop_copy = dict(controller_state.managed_tools_details)
        ...
        if ecu_state_data is not None:
            try:
                ...
                current_measurement = np.linalg.norm(ecu_state_array) if ecu_state_array.size > 0 else 0.0
                last_ecu_state_to_store = ecu_state_data
                logger.debug(f"[ControlLoop] ECU State Norm={current_measurement:.3f}")
            except (ValueError, TypeError) as e:
                logger.error(f"Error al procesar datos de ECU: {e} - Data: {ecu_state_data}")
                current_measurement = 0.0
                last_ecu_state_to_store = []
        else:
             logger.warning("[ControlLoop] No se pudo obtener estado válido de ECU, usando medición 0.0")
             current_measurement = 0.0
             last_ecu_state_to_store = []
        ...
        if hasattr(controller_state, 'pid_controller'):
             pid_output = controller_state.pid_controller.compute(current_sp_norm, current_measurement, dt)
             logger.debug(f"[ControlLoop] SetpointNorm={current_sp_norm:.3f}, Measurement={current_measurement:.3f}, PID Output={pid_output:.3f}")
        else:
             logger.error("Instancia PID no encontrada en controller_state.")

        # 4. Calcular Pesos y Enviar Señales de Control Específicas
        control_weights = [1.0 / num_control_axes] * num_control_axes # Default: pesos iguales
        if len(current_setpoint_vector) >= num_control_axes:
            relevant_setpoint_components = [current_setpoint_vector[i] for i in range(num_control_axes)]
            ...
            if sum_abs_components > 1e-9:
                ...
                logger.debug(f"[ControlLoop] SetpointVector (relevant): {relevant_setpoint_components}, Calculated Weights: {control_weights.tolist()}")
            else:
                logger.debug(f"[ControlLoop] SetpointVector componentes relevantes cero. Usando pesos iguales.")
                # Mantener pesos iguales ya asignados
        else:
            logger.warning(f"[ControlLoop] SetpointVector (len={len(current_setpoint_vector)}) "
        ...
        for name, details in managed_tools_loop_copy.items():
            ...
            tool_naturaleza = details.get('naturaleza') ### NUEVO: Obtener naturaleza ###

            if not tool_url:
                logger.error(f"No hay URL definida para '{name}'. Saltando.")
                current_tools_state[name] = {"status": "error", "message": "URL no definida"}
                continue
        ...
        if control_index is not None:
            ...
            if tool_naturaleza == "potenciador":
                ...
                # Si PID pide aumentar (>0), enviar señal negativa (reducir menos).
                signal_to_send = -base_signal # Invertir señal base
                log_nature_logic = f"signal = -base ({-base_signal:.3f})"
            elif tool_naturaleza == "modulador" or tool_naturaleza == "actuador":
                # Pasar señal base, el tool la interpreta.
                signal_to_send = base_signal
                log_nature_logic = f"signal = base ({base_signal:.3f})"
            elif tool_naturaleza == "sensor" or tool_naturaleza == "convertidor":
                # No controlar activamente.
                signal_to_send = 0.0
                log_nature_logic = "signal = 0.0 (pasivo)"
            else:
                # Naturaleza desconocida.
                logger.warning(f"[CtrlDiff] Naturaleza '{tool_naturaleza}' desconocida para '{name}'. Enviando señal 0.0.")
                signal_to_send = 0.0
                log_nature_logic = "signal = 0.0 (desconocido)"
                # --- FIN: Lógica basada en tool_naturaleza ---

                logger.debug(f"[CtrlDiff] Tool '{name}' (Aporta: {tool_aporta_a}, Nat: {tool_naturaleza}, Idx: {control_index}): "
            ...
            logger.warning(f"[CtrlDiff] Afinidad '{tool_aporta_a}' para tool '{name}' no mapeada. Enviando señal 0.0.")
            signal_to_send = 0.0
        ...
        with controller_state.lock:
            ...
            # Actualizar estado y control para cada tool en la estructura principal
            for name, state_data in current_tools_state.items():
                 if name in controller_state.managed_tools_details:
                     controller_state.managed_tools_details[name]['last_state'] = state_data
            for name, control_val in control_signals_sent.items():
                 if name in controller_state.managed_tools_details:
                     # Guardar el valor enviado, o un marcador si falló
                     controller_state.managed_tools_details[name]['last_control'] = control_val if control_val is not None else 'send_failed'
        ...
        if sleep_time > 0:
            ...
            logger.warning(f"El ciclo de control ({elapsed_time:.3f}s) excedió el intervalo ({dt}s).")
...
def health_check():
    ...
    control_loop_alive = any(t.name == "HarmonyControlLoop" and t.is_alive() for t in threading.enumerate())
    return jsonify({
...
def get_harmony_state():
    ...
    return jsonify({"status": "success", "data": controller_state.get_state_snapshot()}), 200
...
def set_harmony_setpoint():
    ...
    try:
        if new_vector is not None and isinstance(new_vector, list):
            ...
            return jsonify({"status": "success", "message": f"Setpoint actualizado a norma({val:.3f}) desde vector.", "new_setpoint_value": val, "new_setpoint_vector": vec.tolist()}), 200
        elif new_value is not None:
            val = float(new_value)
            controller_state.update_setpoint(val, None)
            return jsonify({"status": "success", "message": "Setpoint actualizado a valor.", "new_setpoint_value": val}), 200
        else:
            return jsonify({"status": "error", "message": "Se requiere 'setpoint_value' o 'setpoint_vector' en el JSON."}), 400
    except (ValueError, TypeError) as e:
        logger.error(f"Error al procesar nuevo setpoint: {e}")
        return jsonify({"status": "error", "message": f"Error en el formato de los datos: {e}"}), 400
    except Exception as e:
...
def register_tool_from_ai():
    ...
    Espera JSON: {"nombre": str, "url": str, "aporta_a": str, "naturaleza": str} ### MODIFICADO ###
    """
    data = request.get_json()
    if not data:
    ...
    if not nombre or not isinstance(nombre, str):
        return jsonify({"status": "error", "message": "Campo 'nombre' ausente o inválido."}), 400
    if not url or not isinstance(url, str):
         return jsonify({"status": "error", "message": "Campo 'url' ausente o inválido."}), 400
    if not aporta_a or not isinstance(aporta_a, str):
         return jsonify({"status": "error", "message": "Campo 'aporta_a' ausente o inválido."}), 400
    ### NUEVO: Validar naturaleza ###
    if not naturaleza or not isinstance(naturaleza, str):
         return jsonify({"status": "error", "message": "Campo 'naturaleza' ausente o inválido."}), 400
    ...
    #     return jsonify({"status": "error", "message": f"Naturaleza '{naturaleza}' no reconocida."}), 400

    try:
        # Pasar naturaleza al método de registro
        controller_state.register_managed_tool(nombre, url, aporta_a, naturaleza) ### MODIFICADO ###
        return jsonify({"status": "success", "message": f"Tool '{nombre}' registrado/actualizado en Harmony Controller."}), 200
    except Exception as e:
        logger.exception(f"Error al registrar tool '{nombre}' desde AgentAI.")
        return jsonify({"status": "error", "message": "Error interno al registrar el tool."}), 500
...
def reset_pid():
    ...
    try:
        if hasattr(controller_state, 'pid_controller'):
             ...
             return jsonify({"status": "error", "message": "Controlador PID no encontrado."}), 500
    except Exception as e:
...
def main():
    control_thread = threading.Thread(target=harmony_control_loop, daemon=True, name="HarmonyControlLoop")
    control_thread.start()
    port = int(os.environ.get('HC_PORT', 7000))
    logger.info(f"Iniciando servidor Flask para Harmony Controller en puerto {port}...")
