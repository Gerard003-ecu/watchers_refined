...
try:
    ...
    logger.info(
        "Setpoint inicial (norma): %.3f (de %s)",  # Shortened
        setpoint_init,
        setpoint_vector_init.tolist(),
    )
except (json.JSONDecodeError, ValueError):
    log_msg_setpoint_err = (
        "Error al procesar HC_SETPOINT_VECTOR: %s. Usando setpoint=1.0"
    )
    logger.error(log_msg_setpoint_err, SETPOINT_VECTOR_JSON)
    setpoint_vector_init = np.array(
        [1.0, 0.0]
...
class HarmonyControllerState:
    def __init__(
        ...
        # Clave: nombre_tool, Valor: Dict con 'url', 'aporta_a', 'naturaleza',
    ...
    def register_managed_tool(
        ...
        with self.lock:
            if nombre not in self.managed_tools_details:
                ...
                logger.info(
                    log_msg, nombre, url, aporta_a, naturaleza
                )  # MODIFICADO
            else:
                logger.info(
                    "Actualizando info del tool gestionado: '%s'", nombre
                )
        ...
        try:
            ...
            if data.get("status") == "success":
                ...
                if isinstance(state_list, list) and all(
                    ...
                    log_msg_ecu = "Estado ECU recibido: %sx%s puntos"
                    logger.debug(
                        log_msg_ecu,
                        len(state_list),
                        len(state_list[0]) if state_list else 0,
                    )
                    return state_list
                else:
                    log_msg_ecu_err = "Clave 'estado_campo_unificado' no es lista de listas: %s"
                    logger.error(log_msg_ecu_err, type(state_list))
            else:
                log_msg_ecu_warn = "Respuesta de ECU no exitosa: %s"
                logger.warning(
                    log_msg_ecu_warn,
                    data.get("message", "Formato desconocido"),
                )
        except BaseException as e:
            err_type_name = type(e).__name__
            log_msg_ecu_ex = (
                "Error obtener/procesar ECU (%s) intento %s: %s - %s"
            )
            logger.error(
                log_msg_ecu_ex, url, attempt + 1, err_type_name, e
            )
...
def get_tool_state(tool_name: str, base_url: str) -> Dict[str, Any]:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            response.raise_for_status()
            data = response.json()
            data_preview = str(data.get("state", data))[:70] + "..."
            logger.debug("Estado recibido de %s: %s", tool_name, data_preview)
            return data.get("state", {"status": "success", "raw_data": data})
        except Exception as e:
            err_type_name = type(e).__name__
            log_msg_tool_warn = (
                "Error obtener estado %s (%s) intento %s: %s - %s"
            )
            logger.warning(
                log_msg_tool_warn,
                tool_name,
...
def send_tool_control(tool_name: str, base_url: str, control_signal: float):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            response.raise_for_status()
            logger.info(
                "Señal control %.3f enviada a %s. Respuesta: %s",
                control_signal,
                tool_name,
            ...
            err_type_name = type(e).__name__
            log_msg_send_err = (
                "Error enviar control a %s (%s) intento %s: %s - %s"
            )
            logger.warning(
                log_msg_send_err,
                tool_name,
    ...
    while True:
        ...
        if ecu_state_data is not None:
            try:
                ...
                # Truncate potentially long ecu_state_data for logging
                ecu_data_preview = str(ecu_state_data)[:100]
                logger.error(
                    "Error al procesar datos de ECU: %s - Data: %s",
                    e,
                    ecu_data_preview,
                )
                current_measurement = 0.0
            ...
            logger.warning(
                "[ControlLoop] No se pudo obtener estado ECU, usando medición 0.0"
            )
            current_measurement = 0.0
            last_ecu_state_to_store = []
        ...
        if hasattr(controller_state, "pid_controller"):
            ...
            logger.debug(
                "[ControlLoop] SPNorm=%.3f, Meas=%.3f, PIDOut=%.3f",
                current_sp_norm,
                current_measurement,
                pid_output,
            )
        ...
        if len(current_setpoint_vector) >= num_control_axes:
            ...
            if sum_abs_components > 1e-9:
                ...
                logger.debug(
                    "[ControlLoop] SPVector (relevant): %s, Weights: %s",
                    relevant_setpoint_components,
                    control_weights.tolist(),
                )
            else:
                logger.debug(
                    "[ControlLoop] SPVector componentes relevantes cero. "
                    "Usando pesos iguales."
                )
                # Mantener pesos iguales ya asignados
        else:
            log_msg_weights = (
                "[ControlLoop] SetpointVector (len=%s) no tiene suficientes "
                "dimensiones (%s). Usando pesos iguales."
            )
            logger.warning(
                log_msg_weights, len(current_setpoint_vector), num_control_axes
            )
            # Mantener pesos iguales ya asignados
        ...
        if control_index is not None:
            ...
            if tool_naturaleza == "potenciador":
                ...
                log_msg_ctrl_diff = (
                    "[CtrlDiff] Tool '%s' (Aporta: %s, Nat: %s, Idx: %s): "
                    "Weight=%.3f, PIDOut=%.3f, BaseSig=%.3f -> Logic: %s "
                    "-> FinalSignal=%.3f"
                )
                logger.debug(
                    log_msg_ctrl_diff,
            ...
            logger.warning(
                "[CtrlDiff] Afinidad '%s' para tool '%s' no mapeada. "
                "Enviando señal 0.0.",
                tool_aporta_a,
                name,
            )
...
def register_tool_from_ai():
    ...
    # naturalezas_validas = [
    #     "potenciador", "reductor", "modulador",
    #     "sensor", "actuador", "convertidor"
    # ]
    # if naturaleza not in naturalezas_validas:
    #     return jsonify({
    #         "status": "error",
    #         "message": f"Naturaleza '{naturaleza}' no reconocida."
    #     }), 400
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "status": "success",
                    "message": f"Tool '{nombre}' registrado/actualizado.",
                }
            ),
            200,
        )
